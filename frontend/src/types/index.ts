...
export const STATUT_LABELS: Record<StatutCampagne, string> = {
  brouillon: 'Brouillon', en_cours: 'En cours', terminee: 'Terminée',
  partielle: 'Partielle', annulee: 'Annulée',
};

export const TX_STATUT_LABELS: Record<StatutTransaction, string> = {
  en_attente: 'En attente', envoye: 'Envoyé', confirme: 'Confirmé',
  echec: 'Échec', double_detected: 'Doublon',
};

export interface Agent {
  id: number; nom: string; prenom: string; telephone: string;
  role: Role; quota_gb: number; forfait_label: string | null;
  prix_cfa: number; actif: number; created_at: string; updated_at: string;
}
...
export interface Transaction {
  ...
  nom?: string; prenom?: string; role?: Role;
  telephone: string; montant_fcfa: number | null;
  option_used: 'argent'; statut: StatutTransaction;
  airtel_transaction_id: string | null; airtel_reference: string | null;
  airtel_status: string | null; airtel_message: string | null;
  airtel_raw_response: string | null; accuse_recu_le: string | null;
