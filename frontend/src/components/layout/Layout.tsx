...
export function Layout() {
  const { user, logout, isSuperAdmin, isAssistantAppro, isViewer } = useAuth();
  ...
  const SidebarContent = () => (
    <>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {NAV.filter(({ to }) => {
          if (isAssistantAppro) {
            return ['/campagnes', '/historique', '/compte'].includes(to);
          }
        ...
        {isViewer && (
          <>
            <div className="pt-3 pb-1">
              <p className="px-3 text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Viewer</p>
            </div>
            <NavLink to="/reclamations"
              className={({ isActive }) => clsx(
                'flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all',
